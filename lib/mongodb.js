...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  if (filter && filter.group) {
    ...
    var pipeline = this.buildPipeline(filter, query)
  }
  var fields = filter.fields;
  var groups = filter.group;
  if (groups && fields) {
    this.execute(model, 'aggregate', pipeline, fields, processAggregationResponse);
  } else if (fields) {
...
MongoDB.prototype.modifyFilter = function (filter) {
  ...
  if (filter.order) {
    ...
    filter.order.forEach(function (e) {
      if (filter.group.groupBy && filter.group.groupBy.indexOf(e) === -1) {
...
MongoDB.prototype.buildPipeline = function (filter, query) {
  var pipeline = [];
...
MongoDB.prototype.fromRow = function (model, rowData) {
  ...
  Object.keys(data['_id']).forEach(function (key) {
    data[key] = data['_id'][key]
  });
  delete data['_id'];
  return data;
};
